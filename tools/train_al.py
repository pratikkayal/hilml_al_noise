...
def argparser():
    ...
    parser.add_argument('--noise', dest='noise', help='Noise Type', required=False, type=float)
...
def main(cfg):
    ...
    if(cfg.DATASET.NOISE):
        dataset_out_dir = os.path.join(cfg.OUT_DIR, cfg.DATASET.NAME + "_noise_" + str(cfg.DATASET.NOISE), cfg.MODEL.TYPE)
    else:
...
if __name__ == "__main__":
    ...
    # Check if noise is present in argparse().parse_args())
    if(argparser().parse_args().noise):
        cfg.DATASET.NOISE = argparser().parse_args().noise
